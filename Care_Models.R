...
Care_Model <- map2stan(
  alist(
    ...
    logit(p) <- ap + s_hrs + gp_id[id1] + rp_id[id2] + ap_dyad[dyad] +
      ...
      bp_prtflat*prtflat + bp_prtpreg*prtpreg + bp_prtswol*prtswol,
    ...
    log(lambda) <- al + s_hrs + gl_id[id1] + rl_id[id2] + al_dyad[dyad] +
      ...
      bl_prtflat*prtflat + bl_prtpreg*prtpreg + bl_prtswol*prtswol,
    ...
    c(bp_mom,bp_sis,bp_gma,bp_aunt,bp_distant,
      ...
      bp_enk,bp_ynt,bp_elo1,bp_elo2,bp_elointer,bp_prtflat,bp_prtpreg,bp_prtswol,
      bl_enk,bl_ynt,bl_elo1,bl_elo2,bl_elointer,bl_prtflat,bl_prtpreg,bl_prtswol) ~ dnorm(0,1),
...
caremodeloutput=precis(Care_Model)@output
write.csv(caremodeloutput, file = "care_model output.csv")
...
link2flat <- link(Care_Model , n=1000 , data=d.pred.flat, replace=
                    list(ap_id=a_foc_z , ap_dyad=a_dyad_z, al_id=a_foc_z , al_dyad=a_dyad_z), WAIC=TRUE)
pred.flat <- link2flat$lambda*(1-link2flat$p)
median(pred.flat[,1])
HPDI(pred.flat[,1])

link2lact <- link(Care_Model , n=1000 , data=d.pred.lact, replace=
                    list(ap_id=a_foc_z , ap_dyad=a_dyad_z, al_id=a_foc_z , al_dyad=a_dyad_z), WAIC=TRUE)
pred.lact <- link2lact$lambda*(1-link2lact$p)
median(pred.lact[,1])
HPDI(pred.lact[,1])

link2preg <- link(Care_Model , n=1000 , data=d.pred.preg, replace=
                    list(ap_id=a_foc_z , ap_dyad=a_dyad_z, al_id=a_foc_z , al_dyad=a_dyad_z), WAIC=TRUE)
pred.preg <- link2preg$lambda*(1-link2preg$p)
median(pred.preg[,1])
HPDI(pred.preg[,1])

link2swol <- link(Care_Model , n=1000 , data=d.pred.swol, replace=
                    list(ap_id=a_foc_z , ap_dyad=a_dyad_z,al_id=a_foc_z , al_dyad=a_dyad_z), WAIC=TRUE)
...
dens(pred.lact[,1], xlim=c(0,.5) , ylim=c(-.035,15) , xlab="" , ylab="" , col="white"  , xaxt='n' ,  yaxt='n', zero.line = FALSE , cex.lab=1.5 , cex.axis=0.5 , adj=0.1)
...
legend(.3,13, legend = c("", "", "", ""),
       col=c(1,1)  , lty=c(1,2,3,4),
       lw=1 , cex=.85, bty="n")

legend(.32,13,, legend = c("lactating", "pregnant", "swollen", "flat"),
...
mtext("Rate of aggression from lactating females" , side=1 , line=.3, outer=TRUE , cex=1.5)
axis(1, at = seq(from=0 , to=1, by = .1) ,tck=-0.05 , labels=T )


#limit to lactating vs lactating 

ddsub <- subset(dd, prtlact==1)
...
Care_model_lactvlact<- map2stan(
  alist(
    ...
    c(bp_mom,bp_sis,bp_gma,bp_aunt,bp_distant,
      bl_mom,bl_sis,bl_gma,bl_aunt,bl_distant,
      bp_enk,bp_ynt,bp_elo1,bp_elo2,bp_elointer,bp_samePA,
      bl_enk,bl_ynt,bl_elo1,bl_elo2,bl_elointer,bl_samePA) ~ dnorm(0,1),
    c(sigma_id1,sigma_id2,sigma_dyad) ~ dexp(1),
    c(Rho_id1,Rho_id2,Rho_dyad) ~ dlkjcorr(3)
...
caremodelPAlactvlactoutput=precis(Care_model_lactvlact)@output
write.csv(caremodelPAlactvlactoutput, file = "care_model lactvlact PA output.csv")
...
link2PA <- link(Care_model_lactvlact , n=1000 , data=d.pred.PA, replace=
                  list(ap_Focal=a_foc_z , am_Focal=a_foc_z ), WAIC=TRUE)
pred.PA <- (1-link2PA$p)*link2PA$lambda
median(pred.PA[,1])
HPDI(pred.PA[,1])
median(pred.PA[,2])
HPDI(pred.PA[,2])
...
dens(pred.PA[,1], xlim=c(0,.5), xlab="Rate of aggression from lactating to lactating females", ylim=c(0,10), ylab="",col="white" , yaxt='n', xaxt='n', zero.line=FALSE, cex.lab=1.5, cex.axis=0.5)
ll <- ddsub$aggr1[ddsub$samepa==0]
points(ll, rep(-.01,length(ll)), pch=15 , col=col.alpha("orange1", alpha=0.1) , cex=0.75)

shade( density(pred.PA[,1]) , lim= as.vector(HPDI(pred.PA[,1], prob=0.9999)) , col = col.alpha("orange1", 0.5))
shade( density(pred.PA[,2]) , lim= as.vector(HPDI(pred.PA[,2], prob=0.9999)) , col = col.alpha("#33CCFF", 0.5))
ll <- ddsub$aggr1[ddsub$samepa==1]

points(ll, rep(-.03, length(ll)), pch=17 , col=col.alpha("#33CCFF", alpha=0.1) , cex=0.75)
axis(1, at = seq(from=0 , to=15, by = .1) ,tck=-0.02 , labels=T )
#axis(1, at = seq(from=0 , to=15, by = .5) ,tck=-0.01 , labels=F)
abline(v=median(pred.PA[,1]) , lty=1)
abline(v=median(pred.PA[,2]) , lty=2)

legend(.3,8, legend = c("", ""),
       col=c(1,1)  , lty=c(1,2),
       lw=1 , cex=.85, bty="n")

legend(.32,8,, legend = c("Different PA", "Same PA"),
