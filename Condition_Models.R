...
pred <- link(condition_Pregnant, n=1000 , data=d.pred_elo, replace=
               list(ap_id=a_foc_z , al_id=a_foc_z, WAIC=TRUE))
pred.median <- apply( pred , 2 , median )
pred.HPDI <- apply( pred , 2 , HPDI )
...
pred.lines = pred[sample(nrow(pred),100,replace=F),]
for (i in 1:100){
  lines( timeP.seq , pred.lines[i,] ,lwd=2,col=alpha("#33CCFF",0.05))
}
lines( timeP.seq , pred.median , lwd=1,col="black")
lines( timeP.seq , pred.HPDI[1,],lty=2,lwd=1)
lines( timeP.seq , pred.HPDI[2,],lty=2,lwd=1)
axis(1, at = seq(from=-5 , to=5, by = 1) ,tck=-0.02 , labels=T )
axis(2, at = seq(from=-4 , to=5, by = 1) ,tck=-0.02 , labels=T )
...
pred <- link(condition_Lactating, n=1000 , data=d.pred_elo, replace=
               list(ap_id=a_foc_z , al_id=a_foc_z, WAIC=TRUE))
pred.median <- apply( pred , 2 , median )
pred.HPDI <- apply( pred , 2 , HPDI )
...
pred.lines = pred[sample(nrow(pred),100,replace=F),]
for (i in 1:100){
  lines( timeL.seq , pred.lines[i,] ,lwd=2,col=alpha("#33CCFF",0.05))
}
lines( timeL.seq , pred.median , lwd=1,col="black")
lines( timeL.seq , pred.HPDI[1,],lty=2,lwd=1)
lines( timeL.seq , pred.HPDI[2,],lty=2,lwd=1)
axis(1, at = seq(from=-5 , to=5, by = 1) ,tck=-0.02 , labels=T )
axis(2, at = seq(from=-4 , to=5, by = 1) ,tck=-0.02 , labels=T )
